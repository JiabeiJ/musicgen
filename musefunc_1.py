...
def load_seed_MIDI(select_seed_MIDI: object) -> object:
    f = ''
...
def preprocess_events_matrix(score):
    events_matrix = []
    itrack = 1
    ...
    return events_matrix1

def compose_melody_chords(events_matrix1, number_of_prime_tokens):
    melody_chords = []
    pe = events_matrix1[0]
    for e in events_matrix1:
        ...
        pe = e
    return melody_chords[:number_of_prime_tokens]



def melody(filepath, number_of_prime_tokens):
    score = TMIDIX.midi2single_track_ms_score(open(filepath, 'rb').read(), recalculate_channels=False)
    melody_chords_f = []
    ...
    for ss in song:
        ...
        if ss >= 1280 and ss < 2816:
            ...
            return song_f,melody_chords_f


def compose():
    import torch
    model,ctx=instantiate(full_path_to_model_checkpoint, model_precision)
    melody_chords_f =melody(number_of_prime_tokens)
